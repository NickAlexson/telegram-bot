...
def main():
    ...
    app_tg.job_queue.run_daily(
        lambda context: check_sheet(context, SPREADSHEET_ID_1, 'Утренняя проверка Чек листов'),
        time=time(hour=10, minute=10, tzinfo=MOSCOW_TZ)
    )

    # Вечерняя проверка в 21:30 по Москве
    app_tg.job_queue.run_daily(
        lambda context: check_sheet(context, SPREADSHEET_ID_2, 'Вечерняя проверка Чек листов'),
